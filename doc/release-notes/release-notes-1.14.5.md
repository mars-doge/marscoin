...
* the recommended minimum transaction fee is 0.01 MDOG/kb, and
* the recommended dust limit is 1 MDOG, and
* the recommended RBF increment is 0.001 MDOG.

### Wallet/UI Changes

* The user interface for selecting fees when transacting MDOG has been updated
...
* The default minimum transaction fee is now 0.01 MDOG per kilobyte. Note that
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 MDOG, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 MDOG.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 MDOG and sets the value under
